...
> **For installed packages, the live activity docs are authoritative.** This index maps a capability → package → **bundled** fallback references (`references/<service>/…`). When the package is installed, prefer its generated docs at `{PROJECT_DIR}/.local/docs/packages/{PackageId}/` — start at `overview.md`, and read `coded/coded-api.md` for the coded API. Use this index to resolve the package name/version and as a fallback when `.local/docs` has no entry for it. See [SKILL.md § Activity Documentation](../SKILL.md#activity-documentation-primary-api-source).
