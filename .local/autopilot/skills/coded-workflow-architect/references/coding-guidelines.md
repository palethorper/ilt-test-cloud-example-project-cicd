...
**MANDATORY: Before generating any C# code, get the API right — activity docs first, then existing project patterns.**

### Step A: Read Activity Docs (primary, source-accurate)

For every package you intend to call, consult its activity docs **before** inferring APIs from examples or `CodeGenerationPrerequisitesTool`. Start at `overview.md` (the authoritative index) and read the coded API doc it links, `coded/coded-api.md` — a single file with the service accessor, sub-services, method signatures, parameter/return types, enums, and worked C# examples. See [SKILL.md § Activity Documentation](../SKILL.md#activity-documentation-primary-api-source).

```
ReadFileTool:
  filePath: <PROJECT_DIR>/.local/docs/packages/<PackageId>/overview.md          # authoritative index — links the coded doc

ReadFileTool:
  filePath: <PROJECT_DIR>/.local/docs/packages/<PackageId>/coded/coded-api.md   # accessors, signatures, enums, C# examples
```

- Don't know the package? Map the capability via [SERVICE_INDEX.md](SERVICE_INDEX.md), then read that package's `overview.md` → `coded/coded-api.md`.
- Not installed and you need it? Add its **latest** version (new dependency; latest ensures docs). Already installed with no `coded/` folder? **Don't auto-update it to chase docs** — rely on Steps B–C, and at most suggest the user update the package manually.

### Step B: Search for Existing C# Files
...
- [ ] Consulted activity docs (`.local/docs/packages/{PackageId}/coded/`) for the packages involved — or confirmed unavailable and fell back (no auto-update of installed packages)
...
**Fallback only — activity docs (Step 1.3 Step A) are the primary API source.** Call this **ONLY IF** the relevant package has no `coded/` docs AND (fewer than 5 .cs files found OR existing files don't show API usage **for the domain the user requested** — e.g., user asked for UI automation but all 5 files only use Excel).
...
For the complete service-to-package mapping and versions → see [SERVICE_INDEX.md](SERVICE_INDEX.md). Once you know the package, its **coded activity docs** (`.local/docs/packages/{PackageId}/coded/`) are the authoritative reference for that service's accessors and method signatures — SERVICE_INDEX maps capability → package; the docs give the exact API.
...
2. **Type errors** — `CS0246`, `CS0029`, `CS1061`. For a missing member/type (`CS1061`/`CS0246`), the coded API doc (`.local/docs/packages/{PackageId}/coded/coded-api.md`) has the exact accessor, method name, and signature — re-read it rather than guessing.
