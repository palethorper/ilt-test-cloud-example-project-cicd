...
**Class Name:** `UiPath.UIAutomationNext.Activities.NSAPLogin`
...
Run `uip rpa --help` to find the command that retrieves the default XAML for an activity, then run it with the class name `UiPath.UIAutomationNext.Activities.NSAPLogin`.

## Notes

- Before automating SAP, read [`sap-guide.md`](../references/sap-guide.md).
- This activity must be placed inside a `UiPath.UIAutomationNext.Activities.NSAPLogon` or `UiPath.UIAutomationNext.Activities.NApplicationCard` scope.
