...
| `FilePath` | `InArgument<string>` | Cond. | — | Local file path to upload (one of `FilePath` or `FileResource` required). **Max file size: 50 MB.** |
| `FileResource` | `InArgument<IResource>` | Cond. | — | Resource object to upload (alternative to `FilePath`). **Max file size: 50 MB.** |
...
- **Maximum file size is 50 MB.** Uploads larger than 50 MB are rejected by the Data Service API at runtime — split or compress before calling this activity
